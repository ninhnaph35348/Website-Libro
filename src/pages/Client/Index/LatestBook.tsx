...
const LatestBook = () => {
    const { productVariantLatest, fetchLatestVariants } = useContext(ProductVariantContext);
            ...
            <div className="container">
                ...
                <div className="swiper book-slider">
                    <Swiper
                        ...
                        slidesPerView={5} // Hiển thị 4 sản phẩm
                        breakpoints={{
                    ...
                    >
                        {productVariantLatest.length > 0 ? (
                            productVariantLatest.slice(0, 5).map((variant: IProductVariant) => (
                                <SwiperSlide key={variant.id}>
                                    <div className="shop-box-items style-2">
                                        <div className="book-thumb center">
                                            <Link to={`/shop-details/${variant.product.code}`}><img
                                                src={
                                                    variant.product.image
                                                        ? `http://127.0.0.1:8000/storage/${variant.product.image}`
                                                        : news9
                                                }
                                                alt={variant.product.title} /></Link>
                                        ...
                                        <div className="shop-button">
                                            <Link
                                                to={`/shop-details/${variant.product.code}`} className="theme-btn"><i className="fa-solid fa-basket-shopping" />Thêm vào giỏ hàng</Link>
                                        </div>
                                    </div>
                                </SwiperSlide>
                            ))
