...
const EditProduct = () => {
            ...
            try {
                ...
                if (response?.data) {
                    ...
                    reset({
                        ...
                        author_id: authors.find((a: any) => a.name === product.author)?.id || "",
                        publisher_id: publishers.find((p: any) => p.name === product.publisher)?.id || "",
                        language_id: languages.find((l: any) => l.name === product.language)?.id || "",
                        category_id: categories.find((c: any) => c.name === product.category)?.id || "",
                    });

                    // ✅ Cập nhật danh sách thể loại đã chọn từ API
                    setSelectedGenres(
                        genres.filter((g: any) => product.genres.includes(g.name)).map((g: any) => g.id)
                    );
    ...
    return (
        <div className="p-6 bg-gray-100 min-h-screen">
            <h2 className="text-3xl font-bold text-gray-800 mb-6">Chỉnh sửa sản phẩm</h2>
            <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-2 gap-6">
                ...
                <div className="space-y-6 bg-white p-6 rounded-lg shadow-md">
                    ...
                    {[
                        ...
                        <div key={name}>
                            <label className="block text-gray-700 font-medium mb-1">{label}</label>
                            <select {...register(name as keyof IProduct)} className="w-full p-2 border rounded-lg">
                ...
                <div className="w-full text-center col-span-2 flex justify-center gap-4">
                    <button type="button" onClick={() => navigate(-1)} className="bg-gray-500 text-white px-4 py-2 rounded">Quay lại</button>
