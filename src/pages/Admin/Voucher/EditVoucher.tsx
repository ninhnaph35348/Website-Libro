...
const EditVoucher = () => {
    const { onEdit } = useContext(VoucherContext);
    const {
        register,
        handleSubmit,
        reset,
    ...
    useEffect(() => {
        (async () => {
            const voucher = await getVoucherById(param.code as string | number);
            reset({
                code: voucher.code,
                ...
                used: voucher.used,
                valid_from: voucher.valid_from ? `${voucher.valid_from}T00:00` : "",
                valid_to: voucher.valid_to ? `${voucher.valid_to}T00:00` : "",
            });
        })();
    }, [param.code, reset]);
    const discountType = watch("discount_type");

    const onSubmit = async (voucher: IVoucher) => {
        await onEdit(voucher, param.code);
        navigate('..');
    };

    return (
        <div className="p-6 w-full mx-auto bg-white shadow-md rounded-lg">
            <h2 className="text-xl font-bold mb-4">Sửa mã giảm giá</h2>
